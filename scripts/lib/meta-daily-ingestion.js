...
async function ingestBrand(ctx) {
  ...
  for (const raw of rawAds) {
    ...
    try {
      ...
      if (placement.landing_url) {
        const adCopy = [placement.hook, placement.headline, placement.description]
          .filter(Boolean)
          .join(" ")
          .trim();
        if (adCopy) {
          report.tag_candidates.push({
            advertiser: brand.domain,
...
/**
 * Run Meta daily ingestion for one or all watchlist brands.
 * @returns summary with placement insert/update totals per brand
 */
export async function runMetaDailyIngestion(options = {}) {
  const {
    dryRun = false,
    brand: singleBrand = null,
    limit = DEFAULT_LIMIT,
    country = DEFAULT_COUNTRY,
    actorId = process.env.APIFY_META_ACTOR ?? DEFAULT_ACTOR,
  ...
  if (!useFixture && (!supabaseUrl || !serviceKey) && !providedSupabase) {
    throw new Error("SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are required.");
  }
  if (!dryRun && !apifyToken) {
    throw new Error("APIFY_TOKEN is required for live ingestion (or use --dry-run with fixture).");
  }
  ...
  if (!brands.length) {
    throw new Error("No brands found. Add domains to agency_watchlist or pass --brand.");
  }
...
async function main() {
  ...
  try {
    const summary = await runMetaDailyIngestion({
      dryRun,
      brand: singleBrand,
      limit,
    });
